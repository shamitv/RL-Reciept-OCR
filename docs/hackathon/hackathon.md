\# OpenEnv Hackathon — Dashboard / Round 1 Problem Statement
...
Penalizes clearly undesirable behavior (for example: infinite loops, destructive actions).
...
Reads API credentials from environment variables (`OPENAI\_API\_KEY`).
...
\## Additional Sections Visible but Collapsed
